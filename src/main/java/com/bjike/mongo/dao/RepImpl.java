...
import com.bjike.type.DataType;
...
public class RepImpl<Entity extends BaseEntity, Dto extends BaseDTO> implements Rep<Entity, Dto> {
    ...
    public List<Entity> findByPage(Dto dto) {
        ...
        query.skip(dto.getPage() * dto.getLimit());
    ...
    private Query init_search(Query query, Dto dto) {
        ...
        stream.forEach(model -> {
            String[] values = new String[]{model.getValue().toString()};
            RestrictionType restrict = model.getRestrict();
            DataType dataType = DataType.valueOf(PrimitiveUtil.switchType(model.getValue()).getSimpleName().toUpperCase());
    ...
    private Query init_sort(Query query, Dto dto) {
        if (null != dto.getSorts() && dto.getSorts().size() > 0) {
            String sorts[] = dto.getSorts().get(0).split("=");
            Sort.Direction direction = Sort.Direction.DESC;
            if (null != sorts) {
                if (sorts[1].equals("asc")) {
                    direction = Sort.Direction.ASC;
                } else {
                    sorts = new String[]{dto.getSorts().get(0)};
                }
            }
            query.with(new Sort(direction, sorts[0]));
        }
        return query;
    }
